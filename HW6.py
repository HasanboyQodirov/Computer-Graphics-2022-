...
def main():
    ...
    while not done:
        ...
        for i, rect in enumerate(rm.rectangles):
            pygame.draw.rect(
                screen, YELLOW if i == rm.rect_dragging else BLUE, rect, 2
            )
            if i > 0:
                pygame.draw.line(screen, GREEN, rm.rectangles[i - 1].center, rm.rectangles[i].center, 2)

    
        draw_text(f'Points : {len(rm.rectangles)}', YELLOW, (10, 10))
        draw_text(f'Mouse x:{mouse_x} y:{mouse_y}', YELLOW, (10, 30))
        draw_text(f'FPS :{clock.get_fps():.2f} index :{rm.indexing}', YELLOW, (10, 50))
        ...
        if len(rm.rectangles) >= 2:
            ...
            draw_text(f'Draw Time :{calc_time:.2f} ms', YELLOW, (10, 70))
